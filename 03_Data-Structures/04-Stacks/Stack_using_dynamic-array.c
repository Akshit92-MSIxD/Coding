...
int* stack_size_change(int *stack,int *n)
{
      printf("Enter the new size of dynamically allocated stack : ");
      scanf("%d",n);
      stack=realloc(stack,(*n)*(sizeof(int)));
      if(stack==NULL)
      printf("Memory is not reallocated!!!");
      else
      printf("Memory is reallocated succesfully!!!");
      return stack;
}
...
int *stack=(int*)calloc(n,sizeof(int)), top=-1,choice;
...
printf("Enter 7 to change the size of the stack");
           ...
           do{
               ...
               switch(choice)
               {
                   ...
                   case 6: stack_checkFull(&n,&top);
                            break;
                    case 7: stack_size_change(stack,&n);
                             break;
