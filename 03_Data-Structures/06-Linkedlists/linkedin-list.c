...
//Very very important concept in case 2
/* head->data=5;
head->next=second;
...

{
  ...
  // Method 2 : Creating 5 nodes in heap with only two pointers(head and current)
  ...
  current=(struct node*)malloc(sizeof(struct node)); // creating fifth node but assigning its address to same current pointer2
  scanf("%d",&current->data);
  current->next=NULL;
  ...
  struct node* access_ptr=head; //Mote if head==NULL , that means linked list is empty
  int count_node=0;

  printf("\n\nDisplay linked list items : ");
  
  while(access_ptr != NULL) // very important condition to traverse(printing/counting-nodes)
  {
      count_node++;
     printf("%d ",access_ptr->data);
     access_ptr=access_ptr->next;
